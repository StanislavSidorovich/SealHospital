...
- Сохранение в `localStorage`: один ключ `sh.save` = `{version, album, progress, muted, fish, shells, owned, decor, shifts, pet}` (`pet` — свой тюленёнок или `null`, проверяется в `sanitizePet()`; `pet.seen` — когда навещали), объект `save` и `persist()` в `js/data.js`. Новое поле: значение по умолчанию в `sanitize()`; меняется смысл старых данных — поднять `SAVE_VERSION` и добавить функцию в `MIGRATIONS`. Старые ключи `sh.album/progress/muted` только читаются при миграции. Каждое чтение и запись оборачивать в try/catch (`store` уже так делает). Браузеру передаём просьбу «не стирай» через `keepSave()` (`navigator.storage.persist()`). **Код сохранения** (`makeCode()`/`readCode()` в `data.js`, окно `#settings` в `game.js`): `SEAL1.<сумма>.<base64 от save без album>`; любое новое поле сохранения автоматически попадает в код, а старые коды проходят через `upgrade()` (миграции + `sanitize`). Поменялся формат — подними `SAVE_VERSION`, а не `CODE_TAG`.
